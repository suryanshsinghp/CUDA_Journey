//g++ -std=c++17 -o out cpu.cpp && ./out > out_cpu.txt
#include <iostream>
#include <stdlib.h> 
#include <fstream>
...
#define flatIdx(x, y, width) ((y) * (width) + (x)) //from 2d to row major flattened index
#define NO_FILE_WRITE //dont write file when N is large, just for testing


void updateT(float *T, float *T_new, int width) {
...
int main(){
    ...
    for (int step = 0; step < num_steps; ++step) {//avoid post increment to avoid copy
        updateT(T, T_new, N);
        std::memcpy(T, T_new, N * N * sizeof(float)); //T= T_new for next iteration and T_new will be updated
    }
